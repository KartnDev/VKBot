...
class UserMsgController:
    ...
    async def handle_(self, event_sender: UserEventSender):
        self.vk.send_message(type_id="user_id", id=event_sender.user_id, message="статус")

    @RequiredLvl(lvl=2)
    @HandleMessage(msg="!Bye")
    async def handle_bye(self, event_sender: UserEventSender):
        self.vk.send_message(type_id="user_id", id=event_sender.user_id, message="!Bye очередняра")

    @HandleMessage(first_word="!ban")
    @RequiredLvl(lvl=1)
    async def handle_partial(self, event_sender: UserEventSender):
        self.vk.send_message(type_id="user_id", id=event_sender.user_id, message="2E40 OK !ban")

    @HandleMessage(first_word="!unban", words_length=3)
    @RequiredLvl(lvl=1)
    async def handle_partial(self, event_sender: UserEventSender):
        self.vk.send_message(type_id="user_id", id=event_sender.user_id, message="2E40 OK !unban")
