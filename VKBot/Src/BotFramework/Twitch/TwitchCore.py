...
class TwitchCore:
    ...
    def kraken_request(self, method_name: str, uri_args: dict = None, method: str = 'get', json=None, files=None):
        full_url = "{0}/{1}?{2}".format(self._V5_KRAKEN_URL, method_name, '&'.join("{0}={1}".format(key, value)
                                                                                   for key, value in uri_args))
    ...
    @staticmethod
    def _make_request(url, method: str = 'get', json=None, files=None):
        return requests.request(url=url, method=method, json=json, files=files)

    @staticmethod
    async def _make_async_request(url, method: str = 'get', json=None, files=None):
        return await requests_async.request(url=url, method=method, json=json, files=files)
